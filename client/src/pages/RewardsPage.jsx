...
const RewardsPage = () => {
    ...
    if (!loading && !currentUser) {
      ...
      const fetchCompletionAndRewardStatus = async () => {
        try {
          ...
          const completionResponse = await axios.get(`${API_BASE_URL}/api/activities/check-completion`, {
          ...
          setError(
            err.response?.data?.error || "Unable to check activity or reward status. Please try again later."
          );
        } finally {
  ...
  useEffect(() => {
    ...
    const checkRewardStatus = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/rewards/status?qrToken=${qrToken}`);
        ...
        setError(
          err.response?.data?.error || "Failed to verify reward status. Please try again."
        );
      }
    };
  ...
  const handleClaimReward = async () => {
    ...
    try {
      ...
      if (err.response?.status === 409) {
        ...
        setError(
          err.response?.data?.error || "Failed to generate reward. Please try again."
        );
      }
    } finally {
      setLocalLoading(false);
  ...
  const isButtonEnabled = completionStatus.allCompleted && !qrCodeUrl && !localLoading && !hasRedeemed;
  ...
  if (loading || localLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col justify-center items-center px-4 sm:px-8">
        <h1 className="text-2xl sm:text-4xl font-bold text-white mb-4">Loading...</h1>
      </div>
    );
  }
