...
async function initializeData() {
  try {
    ...
    const permissionsToCreate = [
      {
        permissionName: "access_app",
        description: "Basic access to the application features",
      },
      {
        permissionName: "view_content",
        description:
          "Ability to view content (words, clues, questions, locations, activities)",
      },
      {
      ...
      },
      {
        ...
        description: "Ability to view roles and permissions",
      },
      {
      ...
      {
        permissionName: "manage_words",
        description: "Ability to manage words",
      },
      {
        permissionName: "manage_clues",
        description: "Ability to manage clues",
      },
      {
        permissionName: "manage_questions",
        description: "Ability to manage questions",
      },
      {
        permissionName: "manage_activities",
        description: "Ability to manage activities",
      },
      {
        permissionName: "manage_locations",
        description: "Ability to manage locations",
      },
      {
        permissionName: "manage_all_content",
        description:
          "Ability to create, edit, and delete content (words, clues, questions, locations, activities)",
      },
      {
      ...
      },
      {
        ...
        description: "Ability to manage admin user accounts",
      },
      {
      ...
      },
      {
        ...
        description: "Ability to manage application settings",
      },
    ];
    ...
    for (const perm of permissionsToCreate) {
      ...
      if (!permission) {
        ...
        console.log(
          `Permission '${perm.permissionName}' already exists, skipping.`
        );
      }
    ...
    const rolesToCreate = [
      ...
      },
      {
        ...
        description:
          "Content manager role with permissions to manage all content",
        permissions: [
          "access_app",
          "view_content",
          "view_users",
      ...
      },
      {
        roleName: "moderator",
        description:
          "Moderator role with permissions to manage users and content",
        permissions: [
          "access_app",
          "view_content",
          "view_users",
      ...
      },
      {
        roleName: "admin",
        description:
          "Administrator role with full access to manage users, content, and settings",
        permissions: [
          "access_app",
          "view_content",
          "view_users",
    ...
    for (const roleData of rolesToCreate) {
      ...
      for (const permName of roleData.permissions) {
        ...
        if (!existing) {
          ...
          console.log(
            `Assigned permission '${permName}' to role '${role.roleName}'.`
          );
        } else {
          console.log(
            `Permission '${permName}' already assigned to role '${role.roleName}', skipping.`
          );
        }
      }
    }
    ...
    for (const user of sampleUsers) {
      ...
      if (!existing) {
        ...
        console.log(`Sample user created: ${createdUser.username}`);
      } else {
        console.log(`Sample user '${user.username}' already exists, skipping.`);
      }
    }
    ...
    const activitiesToCreate = [
      ...
      },
      {
        ...
        description:
          "Take a quiz to discover your most suitable SoC diploma based on your personality.",
        route: "/quiz",
      },
      {
        name: "AR Selfie Challenge",
        description:
          "Take a selfie with an AR filter and share it on social media.",
        route: "/face-filter",
      },
    ];
    ...
    const personalityTypes = [
      {
        ...
        description:
          "You have a sharp eye for patterns and enjoy making sense of complex information. You’re naturally curious and driven to uncover insights through data.",
      },
      {
        code: "DCDF",
        name: "Diploma in Cybersecurity and Digital Forensics",
        description:
          "You’re detail-oriented, cautious, and enjoy solving mysteries. You take pride in protecting others and ensuring systems remain secure and resilient.",
      },
      {
        code: "DCS",
        name: "Diploma in Computer Science",
        description:
          "You enjoy building things from the ground up and thinking logically to solve problems. You’re inventive, hands-on, and love creating solutions that make an impact.",
      },
    ];
    ...
    for (const q of quizQuestions) {
      ...
      if (!existing) {
        ...
        console.log(
          `Quiz question '${q.questionText}' already exists, skipping.`
        );
      }
    }
    ...
    const locationsToCreate = [
      {
        ...
        description:
          "A state-of-the-art lab for AI and data analytics projects, with advanced powered computing resources.",
        code: "T2134",
        points: 10,
      },
      {
        ...
        description:
          "A lab for cybersecurity training and digital forensics exercises, equipped with a custom server for simulations.",
        code: "T2035",
        points: 10,
      },
    ];
    ...
    for (let i = 0; i < createdWords.length && i < createdClues.length; i++) {
      ...
      if (!existing) {
        ...
        console.log(
          `Word-clue relationship created for ${createdWords[i].wordText}`
        );
      } else {
        console.log(
          `Word-clue relationship already exists for ${createdWords[i].wordText}, skipping.`
        );
      }
    }
    ...
    for (const puzzleWordData of puzzleWordsData) {
      if (
        ...
        if (!existing) {
          ...
          console.log(
            `Puzzle word placement created for ${
              createdWords[puzzleWordData.wordIndex].wordText
            } in ${createdPuzzles[puzzleWordData.puzzleIndex].title}`
          );
        } else {
    ...
    if (!user) {
      console.error(
        "User with userId: 1 does not exist. Cannot populate UserActivities."
      );
      return;
    }
    ...
    for (const a of activitiesToCreate) {
      ...
      if (existing) {
        ...
        if (!existingUserActivity) {
          ...
          console.log(
            `UserActivities entry created for userId: 1 and activity: ${existing.name}`
          );
        } else {
          console.log(
            `UserActivities entry for userId: 1 and activity: ${existing.name} already exists, skipping.`
          );
        }
      } else {
        console.log(
          `Activity ${a.name} not found, skipping UserActivities entry.`
        );
      }
    }
  } catch (error) {
