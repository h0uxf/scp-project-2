...
import { Camera, CheckCircle, XCircle, Users, Gift, AlertCircle, Scan, RefreshCw, X, Upload, Shield } from "lucide-react";
  ...
  const checkCameraPermission = async () => {
    try {
      ...
      if (!isSecureContext) {
        ...
        setError("Camera access requires HTTPS connection. Please access this page over HTTPS.");
        return false;
      }
    ...
    if (isScanning) {
      const initializeScanner = async () => {
        ...
        try {
          ...
          const cameraId = selectedCamera || (availableCameras.length > 0 ? availableCameras[0].id : undefined);
          const config = {
          ...
          console.log("Starting scanner with camera:", cameraId || "environment"); // Debug: Log camera used
...
const handleImageUpload = async (event) => {
  ...
  try {
    ...
    reader.onload = async (e) => {
      ...
      img.onload = async () => {
        try {
          const result = await Html5Qrcode.getFileScanResult(img);
          if (result) {
            console.log("QR Code from image:", result);
            if (result === lastScannedToken) {
              toast.error("This QR code was already scanned.", {
                style: { fontSize: "14px", padding: "8px 16px" },
              });
              return;
            }
            setLastScannedToken(result);
...
const handleManualInput = async (qrTokenOrUrl) => {
  ...
  const simulateQRScan = async () => {
    ...
    try {
      ...
      console.log("Simulating QR scan with token:", mockToken); // Debug: Log mock token
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-8 px-4 sm:px-6 text-white text-center">
      ...
      {cameraPermission === "denied" && (
        <div className="max-w-2xl mx-auto mb-6 bg-red-600/20 border border-red-500/50 text-red-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-sm">
              This scanner needs camera access to work. Please enable camera permissions in your browser settings.
            </p>
            <button
      ...
      <div className="max-w-2xl mx-auto bg-white/5 border border-white/20 rounded-2xl p-4 sm:p-8 shadow-xl">
        ...
        <div className="relative">
          <div
            id="reader"
            className={`w-full max-w-md mx-auto rounded-lg overflow-hidden mb-4 ${!isScanning ? "hidden" : ""}`}
            style={{ minHeight: "300px" }} // Increased minHeight for better visibility
          ></div>
          {isScanning && (
            <div className="text-sm text-gray-300 mb-4">
              <p>Scanning... Hold the QR code steady and ensure good lighting.</p>
              <p>Position the QR code within the camera frame, about 6-12 inches away.</p>
            </div>
          )}
          {!isScanning ? (
            <div className="text-center">
              <button
                ...
                className={`bg-blue-600 hover:bg-blue-700 text-white px-6 sm:px-8 py-2 sm:py-3 rounded-full transition-all duration-300 font-semibold flex items-center justify-center gap-2 mx-auto mb-4 sm:mb-6 text-sm sm:text-base ${
                  apiLoading || cameraPermission === "denied" ? "opacity-50 cursor-not-allowed" : ""
                }`}
